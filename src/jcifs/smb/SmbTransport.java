...
public class SmbTransport extends Transport implements SmbConstants {
	...
	static synchronized SmbTransport getSmbTransport( UniAddress address, int port,
        ...
        synchronized( CONNECTIONS ) {
            if( SSN_LIMIT != 1 ) {
                ListIterator<SmbTransport> iter = CONNECTIONS.listIterator();
                while( iter.hasNext() ) {
    ...
    LinkedList<?> referrals = new LinkedList<Object>();
    SigningDigest digest = null;
    LinkedList<SmbSession> sessions = new LinkedList<SmbSession>();
    ...
    synchronized SmbSession getSmbSession( NtlmPasswordAuthentication auth ) {
        ...
        ListIterator<SmbSession> iter = sessions.listIterator();
        while( iter.hasNext() ) {
